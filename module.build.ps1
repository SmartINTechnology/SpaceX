<#
.Synopsis
Test consumed by Invoke-Build to test the build locally
.Description
Test consumed by Invoke-Build to test the build locally
.Example
Invoke-build
The module InvokeBuild will retrieve *build* file and process them
.Notes
Version | Date | Author
...
$script:ModuleName = 'SpaceX'

$script:Source = Join-Path -Path $BuildRoot $ModuleName
$script:Output = Join-Path -Path $BuildRoot output
$script:Destination = Join-Path -Path $Output $ModuleName
...
$script:HelpRoot = Join-Path -Path $Output 'help'
...
Task Clean {
    ...
    {
        $null = Remove-Item -Path $Output -Recurse -ErrorAction Ignore
    }
...
Task Specification {
    ...
    $TestResults = Invoke-Gherkin -Path $PSScriptRoot\Spec -PassThru
    if ($TestResults.FailedCount -gt 0)
    {
...
TaskX BuildPSD1 @{
    ...
    Jobs    = {
        ...
        $functions = Get-ChildItem -Path "$ModuleName\Public\*.ps1" |
        Where-Object -FilterScript { $_.name -notmatch 'Tests'} |
        Select-Object -ExpandProperty basename

        Set-ModuleFunctions -Name $ManifestPath -FunctionsToExport $functions

        Write-Output -InputObject "  Detecting semantic versioning"

        Import-Module -Name ".\$ModuleName"
        $commandList = Get-Command -Module $ModuleName
        Remove-Module -Name $ModuleName

        Write-Output -InputObject "    Calculating fingerprint"
        $fingerprint = foreach ($command in $commandList )
        {
            foreach ($parameter in $command.parameters.keys)
            {
                '{0}:{1}' -f $command.name, $command.parameters[$parameter].Name
                $command.parameters[$parameter].aliases | Foreach-Object -Process { '{0}:{1}' -f $command.name, $_}
            }
        }
        ...
        Write-Output -InputObject '    Detecting new features'
        $fingerprint | Where-Object -FilterScript {$_ -notin $oldFingerprint } |
            Foreach-Object -Process {$bumpVersionType = 'Minor'; "      $_"}

        Write-Output -InputObject '    Detecting breaking changes'
        $oldFingerprint | Where-Object {$_ -notin $fingerprint } |
            Foreach-Object -Process {$bumpVersionType = 'Major'; "      $_"}
        ...
        Write-Output -InputObject "  Stepping [$bumpVersionType] version [$version]"
        $version = [version] (Step-Version $version -Type $bumpVersionType)
        Write-Output -InputObject "  Using version: $version"
